...
//    public List<Camera_people_num> getTopCamera_people_numById(int camera_id, int topN);
    ...
    @Select("SELECT * FROM (SELECT camera_people_num.camera_id,camera_people_num.image,camera_people_num.num,camera_people_num.time,camera.camera_place,camera.max_num FROM camera_people_num,camera WHERE camera_people_num.camera_id=#{camera_id} and camera.camera_id=camera_people_num.camera_id ORDER BY time desc LIMIT #{topN}) var ORDER BY time")
    public List<Camera_people_num> getTopCamera_people_numById(int camera_id, int topN);
